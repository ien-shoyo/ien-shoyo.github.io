...
startupKit.uiKitHeader.header23 = function() {
    ...
    $('#play').on('click', function(evt) {
        ...
        var youtube_link = $('#pPlayer').data('youtube');
        if (isMobile.any() && !(typeof(youtube_link) == 'undefined' || youtube_link === null)) {
            window.location.href = youtube_link;
            return;
        }

        //******** Google analytics: click the play botton *******//
        ga('send', 'event', 'button', 'click', 'play-button');

        $('.popup-video').addClass('shown');
        $('.popup-video, .mask').fadeIn('slow', function() {
            $('.popup-video, #pPlayer').attr('src',"//www.youtube.com/embed/90BSPis1Tv8?rel=0&autoplay=1&autohide=1");
        });

        //  90BSPis1Tv8

        $('.mask').on('click', function() {
            $('.popup-video, #pPlayer').attr('src',"");
